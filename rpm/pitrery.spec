...
if [ $1 -eq 1 ] ; then
	if [ -e /usr/bin/archive_xlog_to_wal ] ; then
		...
		echo "remove /usr/bin/archive_xlog_to_wal"
	...
	if [ -e /usr/bin/restore_xlog ] ; then
		...
		echo "remove /usr/bin/restore_xlog_to_wal"
	fi
fi
